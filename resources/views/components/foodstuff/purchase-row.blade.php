...
<tr class="foodstuff-row">
  ...
  <td>
    <div class="input-group">
      ...
      <input type="number" name="price[]" @isset($row) value="{{ old('price.' . $row) }}" @endisset class="form-control foodstuff-price @isset($row) @error('price.' . $row) is-invalid @enderror @endisset">
      <span class="input-group-text foodstuff-price-measurement-unit">
        @isset($row)
          / {{ old('measurement_unit.' . $row) ?? '...' }}
        @else
          / ...
