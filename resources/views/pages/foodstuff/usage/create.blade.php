<x-app-layout title="Tulis Pemakaian Baru">
  @push('styles')
  ...
  <form action="{{ route('foodstuff.usage.store') }}" method="POST">
    ...
    <x-form.floating-input type="date" name="used_at" old="{{ date('Y-m-d') }}" class="mb-3">Tanggal Pemakaian</x-form.floating-input>
      ...
      <tbody>
        @if ($rowsCount = old('rows_count'))
          @foreach (range(0, $rowsCount - 1) as $row)
            <x-foodstuff.usage-row :foodstuffs="$foodstuffs" row="{{ $row }}" />
          @endforeach
        @endif
      </tbody>
    </table>
  ...
  <template id="foodstuffRow">
    <x-foodstuff.usage-row :foodstuffs="$foodstuffs" />
  </template>
  ...
  @push('scripts')
    ...
    <script type="text/javascript">
      ...
      const foodstuffCreateRow = () => {
        const tbody = document.querySelector('#createUsageTable>tbody')
